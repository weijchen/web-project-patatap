...
function onKeyDown(event) {
  if (keyData[event.key]) {
    ...
    var newSquare = new Path.Rectangle((maxPoint * 1) / 2, 40, 30);
    newCircle.fillColor = keyData[event.key].color;
    newSquare.fillColor = "#fff";
    var text = new PointText((maxPoint * 1) / 2 + 20);
...
function onFrame(event) {
  for (var i = 0; i < circles.length; i++) {
    ...
    if (circles[i].area < 1) {
      ...
      squares[i].remove(); // remove the square from the canvas
      squares.splice(i, 1); // remove the square from the array
      texts[i].remove();
      texts.splice(i, 1); // remove the square from the array
    }
  }
}
